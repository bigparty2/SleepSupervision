...
packet::packet(computer computer, packetMesg message, byte seq)
{
    this->SetPacket(computer, message, seq);
}

void packet::SetPacket(computer computer, packetMesg message, byte seq)
{
    ...
    std::memcpy(this->packetData.macOrigin, computer.macAddr.Get(), 6);
    this->packetData.ipv4Origin = computer.ipv4.Get();
    std::memcpy(this->packetData.nameOrigin, computer.name.c_str(), 64);
